...
- **Príloha k správe (#13, overené live):** `ir.attachment/create` berie `vals_list` (pole dictov) s `datas` = base64 obsahu + `mimetype`, vracia zoznam intov (nových id). Potom `discuss.channel/message_post` s `attachment_ids: [<id>]` prílohu naviaže na správu (overené: správa má `attachment_ids`, `ir.attachment/read` vráti správny `name`/`mimetype`/`file_size`). Osirelá `ir.attachment` (bez `res_model`/`res_id`) je neškodná, ale ak `message_post` po vytvorení prílohy zlyhá, zaloguj jej id — nezmizne sama.
