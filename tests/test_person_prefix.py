...
BOT = 1000
MAREK = [2000, 'Ján Novák']      # zapisovateľ = autor správy
...
def test_enrich_no_prefix_keeps_message_author():
    msgs = enrich([mk(1, '<p>- 4h praca</p>')], BOT)   # autor MAREK = Ján Novák
...
def test_settlement_groups_and_bills_under_prefix_name(monkeypatch):
    ...
    msgs = enrich([
        mk(1, '<p>Oľga Testová:</p><p>- 6h výkres</p>', MAREK, '2026-08-12 10:00:00'),
        mk(2, 'uzavierka', ZBYNEK, '2026-08-12 18:00:00'),
    ], BOT)
...
def test_info_bills_under_prefix_name_rate(monkeypatch):
    from vyuct import config
    monkeypatch.setattr(config, 'RATES', {'Oľga Testová': 20.0, 'Ján Novák': 15.0})
    msgs = enrich([mk(1, '<p>Oľga Testová:</p><p>- 6h výkres</p>', MAREK)], BOT)
